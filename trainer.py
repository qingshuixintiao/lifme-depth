...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = network.LiteMono(model=self.opt.model,
        ...
        self.models["depth"] = network.DepthDecoder(self.models["encoder"].num_ch_enc,
                                                     self.opt.scales)
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                self.models_pose["pose_encoder"] = network.ResnetEncoder(
                    self.opt.num_layers,
                ...
                self.models_pose["pose"] = network.PoseDecoder(
                ...
                self.models_pose["pose"] = network.PoseCNN(
        ...
        if self.opt.predictive_mask:
            ...
            self.models["predictive_mask"] = network.DepthDecoder(
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, 4, dataset_usage=0, img_ext=img_ext)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        val_dataset = self.dataset(
            self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, 4, dataset_usage=1, img_ext=img_ext)
